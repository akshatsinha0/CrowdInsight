<template>
  <div class="image-uploader" :class="[
    { 'drag-over': isDragOver },
    `content-${adaptiveState.contentDensity}`,
    `expertise-${adaptiveState.userExpertise}`
  ]" ref="uploaderElement">
    <div class="upload-area" @dragover.prevent="handleDragOver" @dragleave.prevent="handleDragLeave"
      ...
      <input type="file" ref="fileInput" class="visually-hidden" accept="image/*" @change="handleFileSelect">
...
import { adaptiveState, observeElement, unobserveElement } from '@/services/AdaptiveUIService';
...
const uploaderElement = ref<HTMLElement | null>(null);

// Computed enhancement suggestions based on user expertise
const enhancementSuggestions = computed(() => {
  if (adaptiveState.userExpertise === 'beginner') {
    return ['Try uploading a crowd image', 'Photos with clear crowds work best'];
  } else if (adaptiveState.userExpertise === 'intermediate') {
    return ['High-resolution images improve accuracy', 'Try different crowd scenarios'];
  } else {
    return ['Use RAW format for best results', 'Toggle advanced options for fine control'];
  }
});

onMounted(() => {
  if (uploaderElement.value) {
    observeElement(uploaderElement.value);
  }
});

onBeforeUnmount(() => {
  if (uploaderElement.value) {
    unobserveElement(uploaderElement.value);
  }
});
</script>
...
.image-uploader.content-dense {
  padding: var(--space-3);
}

.image-uploader.content-normal {
  padding: var(--space-5);
}

.image-uploader.content-sparse {
  padding: var(--space-7);
}

/* Expertise-based UI adjustments */
.image-uploader.expertise-beginner .supported-formats {
  font-size: var(--font-size-base);
  color: var(--color-cyan-500);
}

.image-uploader.expertise-expert .upload-placeholder h3 {
  font-size: var(--font-size-base);
}

.image-uploader.expertise-expert .advanced-controls {
  display: block;
}
